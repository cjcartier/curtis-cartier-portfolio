...
const SwitchbackSection: FC<ComponentId> = async ({ _id }) => {
  ...
  const query = getComponent(_id, 'switchback').grab$({
    ...
    switchbacks: q('switchbacks').filter().deref().grabOne$('_id', q.string()).nullable(),
  });
    ...
    <div className={root}>
      ...
      {hasArrayValues(switchbacks) && (
        <div className={switchbacksContainer}>
          {switchbacks.map(switchback => switchback && <CaseStudySwitchBack key={switchback} _id={switchback} />)}
        </div>
      )}
    </div>
  );
