...
export const getComponent = (_id: string, _type: string) =>
  q('*').filterByType(_type).filter(`_id == "${_id}"`);
...
export const constructLiteralArray = <T extends string>(
  literals: T[]
): z.ZodLiteral<T>[] => literals.map((literal) => z.literal(literal));
