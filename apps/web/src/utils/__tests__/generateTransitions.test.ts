...
describe('generateTransitions', () => {
  it('should generate transition strings', () => {
    ...
    expect(result).toEqual(['color 500ms ease-in-out', 'transform 500ms ease-in-out']);
  });
  ...
  it('should add comma between transitions', () => {
    ...
    expect(result).toEqual(['color 200ms ease-in-out', 'transform 200ms ease-in-out,', 'opacity 200ms ease-in-out']);
  });
